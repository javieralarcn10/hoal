// Increab Pixel v1.0 | Published By Javier Alarcón | Created By Stuart Yamartino | MIT License
;(function(window, document, pixelFunc, pixelFuncName, pixelEndpoint, versionNumber) {
	...
	}(window, document, window["opix"], "opix", "https://api.increab.com/api/increabpixel.gif", 1));
	var links = document.getElementsByTagName('a');
	var buttons = document.getElementsByTagName('button');
	for (var i = 0; i < links.length; i++) {
		if(links[i].hasAttribute('href')&& !links[i].href.includes(';'))links[i].setAttribute("data-opix-event", "link-click-"+links[i].text.trim())
	}
	for (var i = 0; i < buttons.length; i++) {
	  buttons[i].setAttribute("data-opix-event", "button-click-"+buttons[i].textContent.trim())
	}
